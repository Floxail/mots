...
> **ABANDONNÉ (2026-07-29, avant implémentation)** : en vérifiant le code du plan avant de le committer, un bug structurel a été trouvé — deux mots indépendants choisis pendant le balayage peuvent se retrouver adjacents sans case Description entre eux (obligation colonne d'une ligne précédente + nouveau mot ligne courante), et `slots.js::deriveSlots` les fusionne alors en un seul slot dont la concaténation n'est presque jamais un vrai mot du dico. Mesuré : 74% des slots sans définition trouvée. Un correctif d'adjacence (interdire toute case fraîche adjacente à une case déjà lettrée) réduit l'échec à 19% mais fait chuter la densité lettres à 47.9% (cible ~81%) sans même tout corriger (les cases *forcées* par une obligation existante ne sont pas protégées). Un test de remplissage glouton pur (squelette réel inchangé + `slots.js` inchangé, sans aucun backtracking) a confirmé 0 succès sur 100 tentatives à 15x15 — la fusion n'est pas le seul problème, le backtracking est structurellement nécessaire à cette échelle. Décision actée avec l'utilisateur : revenir à `grid_generator/backtracking.js` (inchangé) avec un budget très augmenté (`scripts/generate-grid.js` : `timeoutMs` 20000→90000, `maxBacktracks` 2000000→5000000) plutôt que poursuivre cette architecture. Conservé ci-dessous comme trace de l'investigation, pas comme design retenu.
